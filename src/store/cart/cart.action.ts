...
const addCartItem = (cartItems: CartItemPreview[], productToAdd: CartItemPreview): CartItemPreview[] => {
  ...
  if (existingCartItem) {
    return cartItems.map((cartItem) => (cartItem.colorId === productToAdd.colorId 
...
const removeCartItem = (cartItems: CartItemPreview[], cartItemToRemove: CartItemPreview): CartItemPreview[] => {
  ...
  if (existingCartItem && existingCartItem.quantity === 1) {
    return cartItems.filter((cartItem) => cartItem.colorId !== cartItemToRemove.colorId);
  }
  
  // return back cartitems with matching cart item with reduced quantity
  return cartItems.map((cartItem) => (cartItem.colorId === cartItemToRemove.colorId 
    ? { ...cartItem, quantity: cartItem.quantity - 1 }
    : cartItem));
};

const clearCartItem = (cartItems: CartItemPreview[], cartItemToClear: CartItemPreview): CartItemPreview[] => cartItems.filter((cartItem) => cartItem.colorId !== cartItemToClear.colorId);
...
export type UpdateCartItems = ActionWithPayload<CART_ACTION_TYPES.UPDATE_CART_ITEMS, CartItemPreview[]>;
...
export const updateCartItems = withMatcher((cartItems: CartItemPreview[]): UpdateCartItems => createAction(CART_ACTION_TYPES.UPDATE_CART_ITEMS, cartItems));
