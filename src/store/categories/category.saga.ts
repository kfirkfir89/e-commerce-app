...
import { getPreviewCategoriesAndDocuments } from '../../utils/firebase/firebase.category.utils';
...
export function* featchUpdateCategory({
  payload: { collectionKey, docKey, newItems, sortOption },
}: FeatchUpdateCategorySucceeded) {
  try {
    yield* put(
      featchUpdateCategorySucceeded(collectionKey, docKey, newItems, sortOption)
    );
  } catch (error) {
    yield* put(featchCategoriesFailed(error as Error));
  }
...
export function* onPreviewFetchCategories() {
  yield* takeLatest(
    ...
    featchPreviewCategoriesAndDocuments
  );
}
...
export function* categoriesSaga() {
  yield* all([call(onPreviewFetchCategories), call(onUpdateCategoryFetch)]);
}
