...
export type FeatchImageUrl = ActionWithPayload<
  UPLOADIMG_ACTION_TYPES.FETCH_UPLOADING_IMGURL,
  string
>;
...
export const featchImageUrl = withMatcher(
  (url: string): FeatchImageUrl =>
    createAction(UPLOADIMG_ACTION_TYPES.FETCH_UPLOADING_IMGURL, url)
);
...
// SELECTOR
const selectUploadImageReducer = (state: RootState): ImageUploadState =>
  state.uploadImg;
...
const UploadInput: FC<ImageProps> = ({
  ...
  const { isLoading, urlList, error } = state;
  ...
  return (
    <div key={`${colorLabel}`} className="flex max-w-xs justify-center pt-2">
      <div className="flex flex-col justify-center gap-2">
        ...
        {error && (
          <span className="flex justify-center font-semibold text-red-600">
            somthing&apos;s goes wrong, try again
          </span>
        )}
