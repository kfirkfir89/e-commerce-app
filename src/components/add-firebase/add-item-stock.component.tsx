...
export const AddItemStock = ({
  ...
  useEffect(() => {
    setStock((prevState) => {
      ...
      sizes.forEach((s) => {
        const existingSizeStockIndex = updatedStock.findIndex(
          (ss) => ss.size === s.label
        );
        if (existingSizeStockIndex !== -1) {
          updatedStock[existingSizeStockIndex] = {
            size: s.label,
            colors: colors.map((c) => {
              const existingColorStockIndex = updatedStock[
                existingSizeStockIndex
              ].colors.findIndex((cs) => cs.value === c.value);
              if (existingColorStockIndex !== -1) {
                return updatedStock[existingSizeStockIndex].colors[
                  existingColorStockIndex
                ];
              }
  ...
  return (
    <div className="mb-1 flex flex-wrap justify-center gap-x-2">
      {/* render each size */}
      {stock.map((item) => {
        return (
          <div
            key={`${item.size}`}
            className="my-1 flex w-fit flex-col shadow-lg"
          >
            <div className="flex w-full items-center justify-center rounded-t-lg bg-white p-1 font-semibold text-gray-700 shadow-lg">
              Size {item.size} stock
            </div>

            <div className="flex w-72 flex-col sm:w-72">
              {item.colors &&
                item.colors.map((c, i) => {
                  return (
                    <div key={`${c.label}inputstock`} className="flex">
                      <div
                        className={`w-full max-w-xs ${c.value} p-2 ${
                          i + 1 === colors.length ? 'rounded-b-lg' : ''
                        }`}
                      >
                        <FormInput
                          onKeyDown={(e) =>
                          ...
                          onChange={(e) => {
                            ...
                            updatedStock[sizeIndex].colors[colorIndex].count =
